...
(H2 / DIC Fork 1 — Research Proposal · Lab-final scaffold v1.3)
...
> Estimate \(c_{\text{never}}\) **once** as cold-start acquisition on NEVER_LEARNED probe streams only (episodes until success ≥τ for \(w\) consecutive probes); empty pre-dormancy on NEVER_LEARNED = N/A (never coerce to mean≈0). Then lock \(B = 0.5\,c_{\text{never}}\) for all \(\Pr[\text{restore within } B]\), reacquisition, and \(S\) (no per-cell or post-hoc re-fit). Pre-dormancy baselines only for busy/idle/deletion. Old contaminated oracle-sim B = harness-CI only — non-countable.  
...
**Primary (G-D).** G-D Pr := \(P(c\le B)\) on busy (same recovery stream and frozen B as \(S\)); probe-window mean success is diagnostic only, never the kill. Busy arms: \(\Pr(d=200) \le \Pr(d=0) - 0.25\) at \(\kappa \in \{0.25, 0.50\}\), with \(\kappa=1.00\) flat. Idle predicted flat in \(d\). Failure of the busy drop at both \(\kappa \in \{0.25, 0.50\}\) while \(\kappa=1\) is flat → **H2 dies**, bounce to Stack.
...
**\(B\) protocol (Stack alternate \(c_{\text{never}}\)):** NEVER_LEARNED has no pre-dormancy — empty pre-dormancy = N/A; never coerce to mean≈0 in \(c\)/B. \(c_{\text{never}}\) = cold-start acquisition on NEVER_LEARNED probe streams only (episodes until success ≥τ for \(w\) consecutive probes); pilot once → lock \(B = 0.5\,c_{\text{never}}\); no post-hoc re-fit. Pre-dormancy baselines only for arms with an active block (busy/idle/deletion). Old contaminated oracle-sim B = harness-CI only (non-countable; not §6). Open knobs only: PIT cut dates, one frozen open-weight, batch-1.
...
| \(c_{\text{never}}\) / \(B\) | cold-start on NEVER_LEARNED probes; \(B=0.5\,c_{\text{never}}\) | same \(B\) for Pr, \(c\), \(S\); no mean≈0 coerce; no per-cell re-fit |
...
**[RESULTS WITHHELD — Stack hold]** Current dry-run/oracle-sim JSON is non-countable until P0+P1 green and countable B under cold-start \(c_{\text{never}}\).  
...
*Document control: H2_Research_Proposal_Scaffold_v1.3 · draft section · G-D primary kill · one-shot \(c_{\text{never}}\) then \(B=0.5\,c_{\text{never}}\) · §6 blank · for Supervisor for Build & Write review.*
